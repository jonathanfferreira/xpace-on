...
import { Search, Trophy, BookOpen, Star, Loader2, ArrowUpRight } from 'lucide-react';
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                    {users.map((user) => (
                        <Link
                            key={user.id}
                            href={`/@${user.username}`}
                            className="group bg-[#0a0a0a] border border-[#1a1a1a] hover:border-primary/40 rounded-xl p-5 transition-all duration-300 flex flex-col items-center text-center relative overflow-hidden block"
                        >
                            {/* Hover FX */}
                            ...
                            <div className="mt-auto w-full pt-4 border-t border-[#151515] flex items-center justify-between">
                                <div className="text-center flex-1">
                                    <p className="text-[#555] text-[10px] font-mono uppercase tracking-wider mb-1">XP Relacionado</p>
